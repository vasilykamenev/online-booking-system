...
function buildSystemPrompt(vocabulary: SearchVocabulary, today: string): string {
  return [
    ...
    "- A terse, keyword-style request ('yacht Turkey October 6 people', no articles or verbs) states",
    "  criteria exactly as much as a full sentence does — extract every one of them the same way you",
    "  would from 'I'm looking for a yacht in Turkey in October for 6 people'. Short phrasing is not a",
    "  reason to extract less: 'yacht Turkey October 6 people' must produce vesselTypes containing a",
    "  yacht type, location.country 'Turkey', date.month 10, AND capacity.persons 6 — all four, not",
    "  just whichever one happens to read most like a filter.",
    ...
    "    else to extract.",
    "- When the request names a specific real city, also set location.country to the country that",
    "  city is actually in, using your own general geography knowledge — not only when the request",
    "  states the country explicitly, and not only when the city is in the known-cities list below",
    "  (that list is a spelling reference, not the boundary of what you're allowed to know). 'яхта в",
    "  Дубае' names a city whose country you know: location.city = 'Dubai' AND location.country =",
    "  'United Arab Emirates', not city alone — leaving country null here means later filtering has",
    "  no way to tell 'no country stated' apart from 'stated a city outside our small reference table',",
    "  and silently lets results from an unrelated country through. This is completing a fact the",
    "  request already implies by naming a real place, not inventing a new one — the 'never infer'",
    "  rule above is about criteria the request gives no basis for at all (a price, a date, a guest",
    "  count), not a named place's own geography. Only leave country null when the stated place",
    "  genuinely doesn't determine one on its own (an ambiguous name, or a city you aren't confident",
    "  about).",
