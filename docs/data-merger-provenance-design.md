...
1. **Хранение данных конкурентов.** `search_extracted_listings` персистентно
   ...
   **Ещё не решено, и теперь весомее**: с P3 эта таблица не просто наблюдается
   админом — она напрямую формирует то, что видит пользователь. Запись старше
   `INDEX_FRESHNESS_MS` просто перестаёт использоваться (падает обратно на живой
   фетч), так что показ устаревших данных пользователю не грозит — но таблица
   всё ещё не чистится отдельным job'ом, как и `search_page_cache`, и будет расти
   бессрочно.
...
4. **P3's TTL — единая фиксированная константа, не настоящий freshness-check.**
   `INDEX_FRESHNESS_MS = PAGE_CACHE_MS` (24ч) — тот же практический первый шаг,
   что и решение #3: работает, не обосновано данными о том, как часто реально
   меняются цены/наличие на источниках. ETag/If-Modified-Since (rules-документ
   §38) остаётся отдельным, нерешённым уточнением поверх этого.
